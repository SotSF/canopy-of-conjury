class PatternDiamonds extends CartesianPattern {
  ...
  void runDefault(Strip[] strips) {
    ...
    for (int i = 0; i < diamonds.length; i++) {
      if (i == currentShape) diamonds[i].brightness += 60;
      
      float theta = 2 * PI / numShapes;
      float radius = i % 2 == 0 ? 100 : 50;
      float dist = i % 2 == 0 ? dimension / 3 : dimension / 2;
      image.rotate(theta);
      image.fill(color(diamonds[i].hue, 360,360,diamonds[i].brightness));
      image.beginShape();
  ...
  void visualize(Strip[] strips) {
    ...
    for (int i = 0; i < 12; i++) {  // 12 frequency bands/ranges - these correspond to an octave
      ...
      if (i == 5) {
        ...
        diamonds[bassShape].brightness = round(amplitude * 10);
      }
      if (i == 11) {
       diamonds[trebleShape].hue = int(random(30,50));
        diamonds[trebleShape].brightness = round(amplitude * 10); 
      }
    }
    ...
    image.translate(dimension/2,dimension/2);
    for (int i = 0; i < diamonds.length; i++) {
      if (i == bassShape || i == trebleShape) diamonds[i].brightness += 60;
      
      float theta = 2 * PI / numShapes;
      float radius = i % 2 == 0 ? 100 : 50;
      float dist = i % 2 == 0 ? dimension / 3 : dimension / 2;
      image.rotate(theta);
      image.fill(color(diamonds[i].hue,diamonds[i].brightness,360));
      image.beginShape();
      image.vertex(0,radius);
      image.vertex(20,radius + 30);
      image.vertex(0, radius + dist);
